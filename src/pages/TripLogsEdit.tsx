...
export default function TripLogsEdit({route, navigation}: any) {
    ...
    return (
        <ScrollView style={styles.main}>
            ...
            {SettingsManager.settings.showMapInLogEdit && geoLocation && <View style={{width: "80%" , height: 200, alignSelf: "center"}}>
                <LeafletView
                    mapCenterPosition={geoLocation}
                    mapMarkers={[{position: geoLocation, icon: '<div style="text-align: center">📍<p style="font-size: 20px; margin-top: 0; padding-top: 0; text-align: center">Location</p></div>', size : [32, 32], title: "Location"}]}
                />
            </View>}

            <View style={styles.center}>
                <View style={{margin: 10}}/>
                <TouchableOpacity style={styles.neutralButtonNormal} onPress={getLocation}><Text style={styles.acceptButtonText}>{geoLocation ? "Update" : "Get"} Location</Text></TouchableOpacity>
                {geoLocation && <TouchableOpacity style={styles.neutralButtonNormal} onPress={() => setGeoLocation(undefined)}><Text style={styles.acceptButtonText}>Delete Location</Text></TouchableOpacity>}
                <View style={{margin: 10}}/>
            </View>
