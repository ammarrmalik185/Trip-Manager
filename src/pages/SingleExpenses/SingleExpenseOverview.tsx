...
export default function SingleExpenseOverview({route, navigation}: { route: any, navigation: any }) {
    ...
    return (<View style={styles.main}>
        <PopupModal state={modalVisible}
                    modalData={new ModalData(ModalType.SoftConfirmation, "Are you sure you want to delete this expense?", (confirm: any) => {
        ...
        <Text
            style={styles.dateDisplay}>{SettingsManager.settings.currencySymbol} {route.params.singleExpense.amount}</Text>

        <Text style={styles.expenseContainerDescription}>{route.params.singleExpense.description}</Text>

        <View style={styles.horizontalStack}>
            <TouchableOpacity style={styles.iconTextGroup} onPress={() => {
                navigation.navigate(pages.SingleExpensesEdit, {singleExpense: route.params.singleExpense})
            }}>
                <Image
        ...
        <View style={{...styles.bottom, height: "65%", bottom: 0}}>
            ...
            <TouchableOpacity style={styles.neutralButtonSingleExpense}
                              onPress={() => navigation.navigate(pages.SingleExpensesSettle, {singleExpense: route.params.singleExpense})}>
